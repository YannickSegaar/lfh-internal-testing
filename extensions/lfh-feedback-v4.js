...
export const FeedbackExtension4 = {
  name: 'Feedback',
  type: 'response',
  match: ({ trace }) => trace.type === 'ext_feedback4' || trace.payload?.name === 'ext_feedback4',
  render: ({ trace, element }) => {
    ...
    feedbackContainer.innerHTML = `
      <style>
        /* UNIQUE NAMESPACE: avoid colliding with Voiceflow (.vfrc-*) */
        ...
        /* Ensure no pseudo-elements ever render an extra icon */
      ...
      <div class="romaix-feedback">
        <div class="romaix-feedback__header">
          ...
          <div class="romaix-feedback__buttons">
            <button class="romaix-feedback__button romaix-feedback__button--up" data-feedback="1" aria-label="Thumbs up">
              ${SVG_Thumb}
            </button>

            <button class="romaix-feedback__button romaix-feedback__button--down" data-feedback="0" aria-label="Thumbs down">
              ${SVG_Thumb}
            </button>
        ...
        <div class="romaix-feedback__comment-section">
          <div class="romaix-feedback__comment-label">Would you like to add a comment? (optional)</div>
    ...
    function completeFeedback(payload) {
      ...
      if (!payload.skipped || payload.feedback !== null) sendToWebhook(payload);
    ...
    submitBtn.addEventListener('click', () => {
      ...
      completeFeedback({ feedback: selectedFeedback, comment: comment || null, skipped: false });
    });

    skipBtn.addEventListener('click', () => {
      completeFeedback({ feedback: selectedFeedback, comment: null, skipped: false });
    });
...
function removePreviousFeedbackElements() {
  try {
    ...
    // Remove ONLY our widgets (new class)
...
export default FeedbackExtension4;
