// YRS: LEAD FORM EXTENSION FOR LAST FRONTIER HELISKIING - WIDGET OPTIMIZED - VERSION 2 (20 JAN 2026 - 13:50 CEST)

export const LastFrontierHeliskiingLeadForm_Widget2 = {
  name: 'LastFrontierHeliskiingLeadForm_Widget2',
  type: 'response',
  match: ({ trace }) =>
    trace.type === 'ext_lastFrontierHeliskiingLeadForm_Widget2' ||
    trace.payload?.name === 'ext_lastFrontierHeliskiingLeadForm_Widget2',
  render: ({ trace, element }) => {
    ...
    container.className = 'last-frontier-lead-form-wrapper';
    ...
    container.innerHTML = `
      <style>
        ...
        .last-frontier-lead-form-wrapper {
          width: 100%;
        ...
        .form-group textarea:focus {
          outline: none;
          border-color: ${primaryColor};
          box-shadow: 0 0 0 3px rgba(238, 45, 36, 0.1);
        }
        ...
        .lodge-card:hover {
          ...
          box-shadow: 0 2px 8px rgba(238, 45, 36, 0.1);
        }
        ...
        input[type="range"]::-webkit-slider-thumb {
          ...
          background: ${primaryColor};
          cursor: pointer;
          box-shadow: 0 2px 4px rgba(238, 45, 36, 0.3);
        }
        ...
        .success-icon {
          ...
          align-items: center;
          justify-content: center;
          box-shadow: 0 4px 14px rgba(238, 45, 36, 0.3);
        }
        ...
        .btn-primary:hover {
          transform: translateY(-1px);
          box-shadow: 0 4px 12px rgba(238, 45, 36, 0.35);
        }
    ...
    const groupSizeSlider = container.querySelector('#group-size');
    const groupSizeDisplay = container.querySelector('#group-size-display');
    ...
    const lodgeCards = container.querySelectorAll('.lodge-card');
    ...
    const tripLengthCards = container.querySelectorAll('.trip-length-card');
    ...
    const toggleOptions = container.querySelectorAll('.toggle-option');
    ...
    // Ski days handlers
    const skiDaysCards = container.querySelectorAll('.ski-days-card');
    ...
    function showStep(stepId) {
      ...
      const targetStep = container.querySelector(`#${stepId}`);
      if (targetStep) {
    ...
    function showError(message, step) {
      const errorContainer = container.querySelector(`#error-container-${step}`);
      if (errorContainer) {
    ...
    function validateStep2() {
      const privacyConsent = container.querySelector('#privacy-consent');
    ...
    if (nextBtn) {
      nextBtn.addEventListener('click', async (e) => {
        ...
        if (currentStep === 'form-step-1') {
          ...
          const formData = {
            firstName: container.querySelector('#first-name').value.trim(),
            lastName: container.querySelector('#last-name').value.trim(),
            ...
            hearAboutUs: container.querySelector('#hear-about-us').value,
            bookingTimeframe: container.querySelector('#booking-timeframe').value,
            contactUrgency: container.querySelector('#contact-urgency').value,
            lodge: container.querySelector('input[name="lodge"]:checked')?.value || '',
            tripLength: container.querySelector('input[name="tripLength"]:checked')?.value || '',
            catSkied: container.querySelector('input[name="catSkied"]:checked')?.value || '',
            heliSkied: container.querySelector('input[name="heliSkied"]:checked')?.value || '',
            skiDaysPerYear: container.querySelector('input[name="skiDaysPerYear"]:checked')?.value || '',
            groupSize: container.querySelector('#group-size').value === '13' ? '12+' : container.querySelector('#group-size').value,
            visitDates: container.querySelector('#visit-dates').value.trim(),
            privateTour: container.querySelector('input[name="privateTour"]:checked')?.value || '',
            additionalQuestions: container.querySelector('#additional-questions').value.trim()
          };
...
export default LastFrontierHeliskiingLeadForm_Widget2;
