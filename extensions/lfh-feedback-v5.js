...
export const FeedbackExtension5 = {
  name: 'Feedback',
  type: 'response',
  match: ({ trace }) => trace.type === 'ext_feedback5' || trace.payload?.name === 'ext_feedback5',
  render: ({ trace, element }) => {
    ...
    const {
      ...
      lastUserMessage = '',
      lastAIResponse = '',
      webhookURL = 'https://n8n.romaix-n8n.xyz/webhook-test/1c1e9915-a050-4798-b5e2-fbc10ada63ad',
    } = trace.payload || {};
    ...
    feedbackContainer.innerHTML = `
      <style>
        /* ✅ IMPORTANT: unique namespace to avoid Voiceflow styles colliding */
        ...
        /* ✅ KILL any injected pseudo-icons (belt + suspenders) */
        ...
        .romaix-feedback__comment-input:focus { outline: none; border-color: #2e6ee1; }
        ...
        .romaix-feedback__comment-btn--submit { background: #2e6ee1; color: white; }
        .romaix-feedback__comment-btn--submit:hover { background: #2558b8; }
        .romaix-feedback__comment-btn--skip { background: transparent; color: #666; }
        .romaix-feedback__comment-btn--skip:hover { background: #f0f0f0; }

        .romaix-feedback__add-comment-btn {
          padding: 4px 8px;
          background: transparent;
        ...
        .romaix-feedback__add-comment-btn:hover { background: #2e6ee1; color: white; }
      </style>
      ...
      <div class="romaix-feedback">
        ...
        <div class="romaix-feedback__comment-section">
          <div class="romaix-feedback__comment-label">Add a comment (optional)</div>
          ...
          <div class="romaix-feedback__comment-actions">
            <button class="romaix-feedback__comment-btn romaix-feedback__comment-btn--skip" type="button">Skip</button>
            <button class="romaix-feedback__comment-btn romaix-feedback__comment-btn--submit" type="button">Submit</button>
          </div>
        </div>
      </div>
    `;
    ...
    const skipBtn = feedbackContainer.querySelector('.romaix-feedback__comment-btn--skip');
    const skipMainBtn = feedbackContainer.querySelector('.romaix-feedback__skip-btn');
    const buttonsContainer = feedbackContainer.querySelector('.romaix-feedback__buttons');
    ...
    async function sendToWebhook(feedbackData) {
      const payload = {
        ...
        source: 'Voiceflow Feedback Extension',
        timestamp: new Date().toISOString(),
      };

      console.log('Sending feedback to webhook:', payload);
      ...
      try {
        ...
        else console.log('Feedback sent successfully');
      } catch (error) {
    ...
    feedbackContainer.querySelectorAll('.romaix-feedback__button').forEach((button) => {
      button.addEventListener('click', function () {
        ...
        // Disable buttons and mark selected
        ...
        // Immediately send feedback without comment
        ...
        addCommentBtn.className = 'romaix-feedback__add-comment-btn';
        ...
        buttonsContainer.appendChild(addCommentBtn);

        // Auto-close after 3 seconds if no comment added
        setTimeout(() => {
          if (!commentSection.classList.contains('is-visible') && !feedbackSubmitted) {
            completeFeedback({
              feedback: selectedFeedback,
              comment: '',
              skipped: false,
    ...
    submitBtn.addEventListener('click', () => {
      ...
      // Send updated feedback WITH comment
      sendToWebhook({
        feedback: selectedFeedback,
        comment,
        skipped: false,
...
function removePreviousFeedbackElements() {
  try {
    ...
    // Remove only our widget(s)
    const widgets = chatDialog.querySelectorAll('.romaix-feedback');
...
export default FeedbackExtension5;
