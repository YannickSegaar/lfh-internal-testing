/**
 * Last Frontier Tour Explorer - Variant A: Tour Card Grid
 *
 * In-chat widget displaying a 2x3 card grid of tour packages.
 * Each card shows hero image with overlay mask, tour name,
 * lodge badge, and starting price. Clicking opens the shared modal.
 *
 * Triggers on VoiceFlow trace: ext_tourExplorer or ext_tourExplorer_grid
 *
 * @version 1.0.0
...
export const LFHTourExplorerGrid = {
  name: 'LFHTourExplorerGrid',
  type: 'response',

  match: ({ trace }) =>
    trace.type === 'ext_tourExplorer' ||
    trace.type === 'ext_tourExplorer_grid' ||
    trace.payload?.name === 'ext_tourExplorer' ||
    trace.payload?.name === 'ext_tourExplorer_grid',
...
`;
    ...
    // Card click → open modal focused on tour
    container.querySelectorAll('.lfhte-gw-card').forEach((card) => {
      card.addEventListener('click', () => {
        openTourExplorerModal(card.dataset.tourId);
      });
    });

    // CTA → open modal (no focus)
...
export default LFHTourExplorerGrid;
