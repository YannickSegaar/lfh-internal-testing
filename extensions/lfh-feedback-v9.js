...
export const FeedbackExtension9 = {
  name: 'Feedback',
  type: 'response',
  match: ({ trace }) => trace.type === 'ext_feedback9' || trace.payload?.name === 'ext_feedback9',
  render: ({ trace, element }) => {
    removePreviousFeedbackElements();

    // Cooldown: if feedback just fired within 3 seconds, auto-resolve silently
    const now = Date.now();
    const lastShown = window.__lfhFeedbackLastShown || 0;
    if (now - lastShown < 3000) {
      window.voiceflow.chat.interact({
        type: 'complete',
        payload: { skipped: true, feedback: null },
      });
      return;
    }
      ...
      if (event.data && typeof event.data === 'string') {
        try {
          ...
          if (data.type === 'voiceflow:interact' && !feedbackSubmitted) {
            // User sent a message — their text already resolves the Custom Action.
            // Do NOT send interact() — that would create a phantom second interaction.
            feedbackSubmitted = true;

            if (submitTimer) clearTimeout(submitTimer);
            if (inactivityTimer) clearTimeout(inactivityTimer);

            // Send feedback data to webhook if user had selected feedback
            ...
            // Fade out the widget silently
            const romaixFeedback = feedbackContainer.querySelector('.romaix-feedback');
            if (romaixFeedback) {
              romaixFeedback.classList.add('romaix-feedback--fading');
              romaixFeedback.addEventListener('animationend', () => romaixFeedback.remove());
            }
...
export default FeedbackExtension9;
