...
export const FeedbackExtension8 = {
  name: 'Feedback',
  type: 'response',
  match: ({ trace }) => trace.type === 'ext_feedback8' || trace.payload?.name === 'ext_feedback8',
  render: ({ trace, element }) => {
    ...
    const {
      ...
      autoSubmitDelay = 10, // Initial auto-submit delay in seconds
      inactivityDelay = 10,  // Inactivity threshold after typing stops (in seconds)
    } = trace.payload || {};
    ...
    function resetInactivityTimer() {
      ...
      inactivityTimer = setTimeout(() => {
        // User stopped typing for X seconds - auto-submit with whatever they have
        const comment = commentInput.value.trim();
        
        console.log('Inactivity detected - auto-submitting feedback');
    ...
    commentInput.addEventListener('input', () => {
      // First time user types - cancel the initial auto-submit timer
      ...
      // Reset inactivity timer every time user types
      // This ensures we only submit after they've stopped typing for X seconds
      if (userHasTyped) {
        resetInactivityTimer();
      }
    });

    // Also reset inactivity timer when user focuses the textarea
...
export default FeedbackExtension8;