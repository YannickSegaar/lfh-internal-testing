...
function interactWithAgent(type, payload) {
  try {
    ...
    console.log('[LodgeCompare] interact:', type, payload);
  }
}
...
export function openLodgeCompareModal(focusLodge = null, initialTab = 'overview') {
  ...
  headerBar.innerHTML = `
    <div class="lfhlc-header-left">
      <img src="${LFH_ASSETS.logo}" alt="LFH" class="lfhlc-header-logo" />
  ...
  function renderOverviewTab() {
    ...
    content.querySelectorAll('.lfhlc-overview-card').forEach((card) => {
      card.addEventListener('click', (e) => {
        if (!e.target.closest('.lfhlc-view-lodge-btn')) {
    ...
    content.querySelector('.lfhlc-help-choose-btn')?.addEventListener('click', () => {
      interactWithAgent('lodge_recommendation_request', { source: 'lodge_compare_modal' });
      closeModal();
    });
  }
  ...
  function renderTerrainTab() {
    ...
    content.innerHTML = `
      <div class="lfhlc-terrain-tab">
        ...
        <!-- Side-by-side aerial images -->
        <div class="lfhlc-terrain-images">
          <div class="lfhlc-terrain-card">
            <img src="${bell2.terrain.aerialImage}" alt="Bell 2 terrain" class="lfhlc-terrain-card-img" />
          ...
          <div class="lfhlc-terrain-card">
            <img src="${ripley.terrain.aerialImage}" alt="Ripley Creek terrain" class="lfhlc-terrain-card-img" />
            <div class="lfhlc-terrain-card-label">
    ...
    content.querySelector('.lfhlc-terrain-recommend-btn')?.addEventListener('click', () => {
      interactWithAgent('terrain_recommendation_request', { source: 'lodge_compare_terrain_tab' });
      closeModal();
    });
  }
  ...
  function renderLodgeDetail(lodge) {
    ...
    content.querySelector('.lfhlc-action-ask')?.addEventListener('click', () => {
      interactWithAgent('lodge_inquiry', {
        lodge: lodge.id,
        lodgeName: lodge.name,
        source: 'lodge_compare_modal',
      });
  ...
  function closeModal() {
    interactWithAgent('lodge_compare_closed', {
      lastTab: activeTab,
    });
  ...
  document.addEventListener('keydown', function escHandler(e) {
    if (e.key === 'Escape' && document.getElementById('lfh-lodge-compare-modal')) {
      closeModal();
...
.lfhlc-header-left { display: flex; align-items: center; gap: 12px; }
.lfhlc-header-logo { height: 28px; filter: brightness(0) invert(1); }
.lfhlc-header-title {
...
@media (max-width: 700px) {
  ...
  .lfhlc-tab { padding: 8px 14px; font-size: 11px; }
  .lfhlc-tab-bar { gap: 6px; padding: 10px 16px; overflow-x: auto; }
...
@media (max-width: 500px) {
  ...
  .lfhlc-btn-primary, .lfhlc-btn-outline { width: 100%; }
